...
class AppStore extends Component {
  ...
  clickTabItem = valueId => {
    this.setState({currentTabId: valueId})
  }

  getFilteredApps = () => {
    const {currentTabId} = this.state
    const filteredApps = appsList.filter(
      eachAppDetails => eachAppDetails.category === currentTabId,
    )
    return filteredApps
  }
  ...
  render() {
    ...
    const searchedApp = filteredApps.filter(eachApp =>
